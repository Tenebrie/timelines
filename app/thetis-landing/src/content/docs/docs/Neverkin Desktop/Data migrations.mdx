...
Data version is different from the application version. The **application version** is the version of the Neverkin codebase that is running, and it is updated when a new version of the code is released. The **data version** is the version of the database that is used to store the data, and it is updated when the database schema changes.

All your data is automatically upgraded to the latest data version when you run the application, no extra action is needed. However, you may not be able to open newer data with an older version of the app. Rule of thumb - upgrading is safe, downgrading may or may not work. If you're unsure, always backup your data!

:::note
I am working on decoupling data migration from environmental versions. Stay tuned for updates!
:::
