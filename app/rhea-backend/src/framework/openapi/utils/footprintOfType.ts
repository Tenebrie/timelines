...
function footprintOfTypeWithoutFormatting(params: {
	type: Type
	node: Node
	...
	const next = (nextType: Type, nextFlags: FormatFlags[] = []) => {
		return footprintOfTypeWithoutFormatting({
			type: nextType,
			node,
			overrides,
			flags: nextFlags,
	...
	const indent = (text: string, lvl = 1) => text.replace(/^/gm, ''.repeat(lvl * 2))
...
export { footprintOfTypeWithoutFormatting }
