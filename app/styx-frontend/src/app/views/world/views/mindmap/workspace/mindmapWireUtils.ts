...
/**
 * Given a direction from the rect center toward a target point, pick which of the
 * four fixed anchor points (top/right/bottom/left midpoint) the wire should attach
 * to, rather than sliding continuously along the perimeter.
 */
export function nearestEdgePoint(
	...
	if (scaleX <= scaleY) {
		...
		return { x: cx + nx * halfW, y: cy, nx, ny: 0 }
	}

	const ny = Math.sign(dy)
	return { x: cx, y: cy + ny * halfH, nx: 0, ny }
}
