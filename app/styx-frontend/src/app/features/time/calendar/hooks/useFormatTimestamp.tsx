	...
	return useMemo(() => {
		...
		const parseTimestamp = ({
			...
			outputMap.set(unit.id, {
				value: index + (unit.displayFormat === 'Hidden' ? 0 : extraDuration + franDuration),
				formatShorthand: unit.dateFormatShorthand ?? undefined,
				customLabel,
			})
			...
			for (let i = 0; i < unit.children.length; i++) {
				...
				if (remainder < childUnit.duration * childRelation.repeats) {
					if (childUnit.displayFormat !== 'Hidden' && childUnit.displayName !== lastSeenChildName) {
						myFranDuration = 0
					}
		...
		const formatParsed = (parsed: ParsedTimestamp) => {
			if (!calendar.dateFormat || calendar.dateFormat?.trim().length === 0) {
			...
			const flushCurrent = () => {
				...
				const caseSensitive =
					dateFormat.includes(current.symbol.toLowerCase()) &&
					dateFormat.includes(current.symbol.toUpperCase())
				const unit = calendar.units.find((u) => {
					if (!parsed.get(u.id)) {
						return false
					}

					if (caseSensitive) {
						return u.dateFormatShorthand === current.symbol
					}

					return u.dateFormatShorthand?.toLowerCase() === current.symbol.toLowerCase()
				})
				if (unit) {
